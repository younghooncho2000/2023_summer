...
if __name__ == "__main__":
    ...
    parser.add_argument("--indir", default='/content/drive/MyDrive/A/', type=str)
    #parser.add_argument("--outdir", default='StorySalon/image_inpainted', type=str)
    parser.add_argument("--outdir", default='/content/drive/MyDrive/A/image_inpainted', type=str)
    ...
    outpath_list = [x.replace("A/image", "A/image_inpainted") for x in image_list]
    print(f"Found {len(mask_list)} inputs.")

    config = OmegaConf.load("/content/drive/MyDrive/StoryGen/data_process/SDM/models/ldm/inpainting_big/config.yaml")
    model = instantiate_from_config(config.model)
    model.load_state_dict(torch.load("/content/drive/MyDrive/StoryGen/data_process/SDM/models/ldm/inpainting_big/last.ckpt")["state_dict"], strict=False)
